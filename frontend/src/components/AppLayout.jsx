...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-100">
      {/* Sidebar */}
      <aside
        className={`fixed lg:static z-40 inset-y-0 left-0 w-64 bg-slate-950 text-slate-200 flex flex-col transform transition-transform ${
      ...
      >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-5 py-2.5 text-sm border-l-2 transition-colors ${
                  isActive
                    ? "bg-slate-900 border-[#FF4B00] text-white font-semibold"
        ...
        <div className="border-t border-slate-800 p-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#0F3BE8] flex items-center justify-center text-white font-bold text-sm">
              {user?.name?.[0] || "U"}
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div className="px-2 py-1 bg-slate-900 text-white text-[10px] uppercase tracking-wider font-bold">
              {ROLE_LABEL[user?.role] || user?.role}
            </div>
          </div>
        </header>
